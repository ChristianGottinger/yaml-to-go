...
function yamlToGo(yaml, typename, flatten = true, serializationTags) {
	let data;
	let scope;
	let go = "";
	let tabs = 0;
	...
	serializationTagsArray = serializationTags.trim().split(',').filter(n => n)
	...
	function parseStruct(depth, innerTabs, scope, omitempty) {
		...
		if (flatten && depth >= 2) {
			...
			for (let i in keys) {
				...
				if (serializationTagsArray.length > 0) {
					appender(' `');
					serializationTagsArray.forEach(function (tag) {
						appender(tag + ':"' + keyname);
